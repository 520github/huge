...
// math.mean([1,2,3])
//   => 2
math.avg = math.average = function(arr,decimalNum) {
  if(arr && arr.length>0){
    decimalNum = decimalNum || 0;
    var rs = math.sumForMoney(arr)/arr.length;
    return parseFloat(rs.toFixed(decimalNum));
  }else{
    return '--';
  }
  
};
//add this method for avg money
//keep 2 decimal
math.avgForMoney = function(arr) {
	if(arr && arr.length>0){
	    var rs = math.sumForMoney(arr)/arr.length;
...
//add this method for sum money
//keep 2 decimal
math.sumForMoney = function(obj, key) {
...
//   => 3 (四舍五入显示)
//   TODO {}, [{}]
math.median = function(arr,decimal) {
  if(!arr || arr.length==0) return '--';
  var arr = arr.slice(0); // create copy
  var middle = (arr.length + 1) /2;
  var sorted = math.sort(arr);

  var rs = (sorted.length % 2) ? sorted[middle - 1] : (sorted[middle - 1.5] + sorted[middle - 0.5]) / 2;
  if(decimal){
	  return parseFloat(rs.toFixed(decimal))
  }else{
...
// every num being percented
// math.percent([2,4,3])
//   => [0.22, 0.44, 0.33] 
math.percent = function(arr, decimals , percentNum) {
  var sum = math.sum(arr);
  var decimals = decimals || 2;
  return _.map(arr, function(i) { 
    var rs;
    if(sum != 0) {
      rs = parseFloat((i/sum).toFixed(decimals));
    }else{
      rs = 0;
    }
    if(percentNum){
      rs = math.toPercent(rs,percentNum);
    }
    return rs; 
  });
};
...
//小数转成百分数
math.toPercent = function(num,n){
  n = n || 0;
  return ( Math.round( num * Math.pow( 10, n + 2 ) ) / Math.pow( 10, n ) ).toFixed( n ) + '%';
};

//两个数组相除
math.twoArraysDivide = function(arr1,arr2,num){
  var rs = [],num = num || 0;
  for(var i=0; i< arr1.length; i++){
    if(arr1[i] === 0 || arr2[i] === 0){
      rs[i] = 0;
    }else{
      rs[i] = parseFloat((arr1[i]/arr2[i]).toFixed(num));
    }
  }
  return rs;
};
