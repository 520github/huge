...
public class DynamicCache {
	...
	//优化key，把key优化成功一个hash值
	private Hashtable<String, Object> cacheMap;//存放缓存数据
	
	private static DynamicCache singleton = null; 
	
	private DynamicCache() {
		cacheMap = new Hashtable<String,Object>();
	}
	
	public static DynamicCache getInstance() {  
        if (singleton == null) {  
            singleton = new DynamicCache();  
        }  
        return singleton;  
    } 
    ...
    public Object getCacheData(Object o, String methodName,Object[] parameters,   
        ...
        if(cacheData != null) {
        	System.out.println("get from cache value:" + cacheData.getData());
        }
